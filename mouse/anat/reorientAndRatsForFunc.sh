...
FIXED=${scriptPath}/templates/WHS_0.5_T2w_200um_skullstripped.nii.gz
...
while read PID sex; do
  ...
  while read SID acqdate roix roiy roiz; do
    ...
    if [ ! -f ${MASK} ]; then
      ...
      RATS_MM -t ${tVal} -v ${vVal} -k ${kVal} ${IMG_RESAMP} ${MASK}
    else
      echo "Data has already been processed for ${PIDSTR}"
    fi
