...
if [ ! -d $outputFolder ]; then
  ...
  antsApplyTransforms -d 3 -n BSpline[3] -i ${outputNameBase}_reorient_RPI_denoise.nii.gz -o ${outputNameBase}_reorient_RPI_denoise_coreg.nii.gz -t ${outputNameBase}_coregComposite.h5 -r $template

  # general N4 command used
  echo "Running bias field correction on $filename"
  N4BiasFieldCorrection -d 3 -i ${outputNameBase}_reorient_RPI_denoise_coreg.nii.gz -x ${templateMask} -r 1 -s 4 -c [50x50x50x50,0.0] -b [200,3] -t [0.15,0.01,200] -o [${outputNameBase}_reorient_RPI_denoise_coreg_bfcorr.nii.gz,${outputNameBase}_reorient_RPI_denoise_coreg_bf.nii.gz]
